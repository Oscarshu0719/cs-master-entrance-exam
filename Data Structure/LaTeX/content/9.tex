        ...
        \begin{algorithm}[H]
            \begin{algorithmic}[1]
                \Function{DeleteMinMinMaxHeap}{MinMaxHeap $s$}
                    ...
                    \If {Root has no children} \Comment{Start.}
                        \State Set $x$ as root and exit.
                    \ElsIf {Min is one of root's children $k$}
                        \If {$s[k].data < x.data$}
                            \State \Call{Swap}{$s[k]$, $x$}
                        \Else
                            \State Set $x$ as root.
                        \EndIf
                        \State Exit.
...
\item \begin{theorem}{(154, 155, 156, 158, 161)} B tree： \label{m-wayst}\begin{itemize}
