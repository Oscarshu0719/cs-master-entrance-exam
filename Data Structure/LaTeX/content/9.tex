        ...
        \begin{algorithm}[H]
            \begin{algorithmic}[1]
                \Function{InsertRedBlackTree}{RedBlackTree $s$, Element $x$}
                    \State $n$ := \Call{SearchRedBlackTree}{$s$, $x$} \Comment{If a node has two red children during the path to $n$, change the node to red and two children to black.}
                    \While {Adjacent red nodes exist}
                        \State Rotate. \Comment{Parent is black and two children are red.}
                    \EndWhile
                    \State $n.data$ := $x$
                    \State $n.color$ := red
                    \While {Adjacent red nodes exist}
